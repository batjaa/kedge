...
- **M3.7 — Design refresh, dashboard & i18n** (wedge, added 2026-07-23): the locked **Open Harbor** baseline applied product-wide (light-first default, self-hosted Space Grotesk display, amended tokens — DESIGN.md 2026-07-23); **dashboard upgrade** (projects rail, stats strip + workspace summary endpoint, server-side lifecycle filters, audit-log **activity feed** with review-action instrumentation M5's inbox will reuse); workspace **General** settings; **SaaS marketing landing** on the anonymous home (demo import in the hero; self-host branch unchanged); **web-UI i18n** (en-US source · es-US · mn-MN · de-DE; Accept-Language negotiation + persisted switcher; document content never translated). ✅ a fresh SaaS visit lands on the localized landing and a paste imports a doc; sign in → the dashboard shows counts, filters, and activity in Mongolian, light theme by default.
