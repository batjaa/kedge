...
3. **One Playwright journey** — the M1 demo criterion, end to end: paste a public URL anonymously → rendered doc with a live diagram → share link opens in a second context → signing up claims the doc. Extends the M0 journey pattern (both apps against a scratch database); still deliberately one journey, not a suite.
