...
**Anchor capture is web-owned and pure at its core.** Selection → selector happens in the browser against the version's projection: the selection maps to projection offsets (`start`/`end`), captures `exact` with ~64 chars of `prefix`/`suffix` context and the `heading_path`, and stamps the version's `projection_version`. The correspondence mechanism is pinned (eng review 1A): **the shared pipeline emits projection offsets as data attributes on rendered block/inline nodes at render time** — capture reads offsets off the DOM it already has; there is no second derivation of "what the document is". The mapping logic is a DOM-agnostic pure core (placeholder tokens are unselectable and unspannable) so it is golden-corpus-testable; the thin DOM layer feeds it real Selections. **A selection the core cannot map never dead-ends** (5A): the composer offers to post as a document-level comment instead, and the failure is logged (`anchor.capture_failed`) so breaking selections are learnable. Anchors bind to the document's current version at creation. **Re-anchoring is explicitly M3** — the hypothes.is ladder, `relocated`/`orphaned` transitions, and re-attach UI are out; M2 ships the anchor format, capture, and rendering of anchored threads.

**Reviewer identity: magic links, share-scoped.** The share page offers "verify your email to comment": a signed, short-lived, single-use link delivered by transactional mail (SPEC §12 — log driver in dev/e2e, SMTP self-host, Postmark SaaS; this module introduces the mail dependency, nothing more — the notification system is M5). Verification creates or reuses a lightweight user (no password) and binds it to that share via `share_participants`; Policies resolve every reviewer capability through that binding, so cross-share or cross-document traversal is structurally impossible (the IDOR matrix grows its reviewer-via-share column across every action). The request endpoint is **enumeration-neutral** (7A): identical response whether the email is known, throttled per-email AND per-IP. **Mail transport failure is a first-class state** (4A): "couldn't send — try again" + `magiclink.sent/send_failed` events (add the SPEC §19 registry row in the implementing commit). Anonymous commenting stays off by default with a per-share toggle (SPEC §10.2). **Mention autocomplete is audience-scoped** (6A): reviewers see only participants already visible on this share's threads; workspace members see the workspace roster too.
...
**Suggestions.** A suggestion is a comment carrying `proposed_text` for its thread's anchored selection, rendered as an inline before→after diff. **Inline threads only** (8A) — a document-level thread has no selection for the text to replace. The document author sets `suggestion_status`; transitions are **author-only and reversible** (pending↔accepted↔declined), every change audit-logged, and M4 consumes the current state. Accepted text is stored verbatim for the M4 improve-prompt. No write-back to the source, ever (SPEC non-goal).
...
**The reading + review surface.** The review rail per `docs/designs/review-page.html`: sticky, cards vertically aligned to their anchor positions, highlight-on-hover both directions, a Threads group in the sidebar, and the sticky doc header + TOC with scroll-spy (issue #51). **Ordering is document-position order** (9A): threads sort by `anchors.start` (document-level threads grouped first) and pagination follows the same order — backed by an index on `(document_version_id, start)`. **Crowding rule** (13A): cards stack in position order with leader lines to their anchors; the focused card expands. **Mobile** (13A): the rail hides; inline highlight markers open a bottom-sheet thread view. **Replying to a resolved thread is allowed and does not auto-reopen** (10A) — the composer shows a "resolved — reopen?" hint. Same surface on the authenticated doc page and the share page, differing only by capability. All Tailwind + DESIGN.md tokens; no webfonts.
...
**Pagination & events.** DB-level pagination for threads and comments. **The rail read is one aggregate query per document** (11A): eager-loaded anchors, counts, latest activity, position-ordered — explicitly no per-thread queries. Named events (`thread.created`, `comment.created`, `suggestion.accepted/declined`, `thread.resolved/forked`, `magiclink.sent/send_failed`, `anchor.capture_failed`) + audit-log entries; comment persistence precedes and never depends on any fan-out.
...
Review-mandated additions (eng review 2026-07-15, all carried into ticket acceptance criteria): magic-link **send-failure** state (4A) · **enumeration-neutral** verify with per-email+per-IP throttles (7A) · **capture-failure → doc-level fallback** (5A) · **mention audience-scoping** IDOR rows (6A) · **suggestion transition** semantics incl. concurrent-flip (8A) · **position-order pagination** (9A) · **reply-to-resolved** hint semantics (10A) · **reaction dedupe** (one 👍 per user per comment) · **offset-annotation round-trip** (render-annotation → capture → selector, 1A) · **draft survives reload** journey.
...
- First module whose reviewer flows touch mail: the self-host story needs only `MAIL_MAILER=smtp` (already env-pluggable); **the preview instance gets real SMTP env at M2 deploy time** (12A) so magic links actually deliver there — a deployment-checklist line, no code.
- Eng review completed 2026-07-15: 13 decisions (1A–13A) folded in above; rail virtualization past ~200 threads recorded as accepted debt (TODOS.md).
