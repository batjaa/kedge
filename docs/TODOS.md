...
- [x] ~~Diagram rendering spike~~ — **done 2026-07-03: VALIDATED** (SPEC §6.2 works as designed). `remark-diagrams` plugin converts ```mermaid/```plantuml fences → components before Shiki, for both `.md` and `.mdx`. **Mermaid**: client-side ESM, theme-aware (dark/neutral), never leaves the browser. **PlantUML**: async RSC → Kroki GET (deflate+base64url), Next fetch-cached, `KROKI_URL` env override, SVG embedded via `<img data:>` (no script surface). Both have skeleton + show-source error states. SPEC.md's own architecture/sequence/ER diagrams render. **Findings:** (a) complex diagrams shrink to fit — click-to-zoom (already in §6.2) is genuinely needed, build at M1; (b) product still needs R2-backed SVG cache keyed by source hash (§6.2) — Next fetch cache is per-build, not shared across instances.
