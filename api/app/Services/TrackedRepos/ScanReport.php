 ...
 * queued an import (new path), queued a re-sync (a held path whose upstream blob
 * sha moved, #94), left an unchanged path alone, or failed that path at discovery
 * time (e.g. a doc-creation constraint violation) — plus, for held paths that no
 * longer match/exist upstream, a `missing` flag (the document is untouched, never
 * deleted). Per-outcome counts and the scan's duration ride along. It is built up
 * as the diff runs and serialized once, so it is written atomically at completion
 * and never blocks on the dispatched import/re-sync jobs (per-file blob failures
 * land on the document rows, 4A, not here).
 *
...
final class ScanReport
{
    ...
    public const OUTCOME_RESYNC_QUEUED = 'resync_queued';
    ...
    /**
     * A held path no longer matched/present upstream (#94): flagged only — the
     * document is untouched, never deleted and never auto-refiled.
     */
    ...
    /**
     * @return array{import_queued: int, resync_queued: int, unchanged: int, missing: int, failed: int}
     */
    ...
    /**
     * How many paths the pattern matched in the tree this scan — every per-file
     * outcome except `missing` (a missing path is, by definition, no longer in the
     * tree, so it is reported but never counted as matched).
     */
    public function matched(): int
    {
        return count($this->files) - $this->counts[self::OUTCOME_MISSING];
    }
