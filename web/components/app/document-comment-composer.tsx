    ...
    >
      {composer.failure ? (
        <div className="mb-2 rounded-lg bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-800 ring-1 ring-inset ring-amber-500/20 dark:bg-amber-500/10 dark:text-amber-200">
          {t('composer.selectionNotAnchored')}
        </div>
      ) : null}
      {canSuggest ? (
      ...
      {isSuggestion ? (
        <div className="space-y-2">
          <textarea
            ...
            aria-label={t('composer.suggestedReplacement')}
            disabled={submitting}
          />
          {suggestionUnchanged ? (
          ...
          <MentionTextarea
            ...
            ariaLabel={t('composer.suggestionNote')}
            disabled={submitting}
          />
        </div>
      ) : (
        <MentionTextarea
          ...
          rows={4}
          className={TEXTAREA_CLASS_NAME}
          placeholder={t('composer.writeComment')}
          ariaLabel={t('composer.inlineComment')}
          disabled={submitting}
        />
      )}
      ...
      <div className="mt-3 flex items-center justify-end gap-2">
        ...
        <button
          ...
          aria-label={isSuggestion ? t('composer.submitSuggestion') : t('composer.postComment')}
        ...
        >
          ...
          {isSuggestion ? t('composer.suggest') : t('composer.post')}
        </button>
      </div>
    </div>
  );
